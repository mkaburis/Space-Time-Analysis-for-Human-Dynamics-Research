...
transformed parameters {
  ...
  vector[N] delta;
  vector[N] t_i;
  vector<lower=0, upper=1>[N] searchrate;
  vector<lower=0, upper=1>[N] hitrate;
  ...
  for (i in 1:N) {	
    ...
    searchrate[i] = (successful_search_rate + unsuccessful_search_rate);
    hitrate[i] = successful_search_rate / searchrate[i];
   }
}
...
model {  
  ...
  s ~ binomial(n, searchrate);
  h ~ binomial(s, hitrate);
}
